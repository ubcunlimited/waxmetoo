  ...
  return (
    <>
      ...
      >
        ...
        {/* Mobile Menu */}
        {mobileOpen && (
          <div className="lg:hidden bg-white border-t border-[#D8C6B6] shadow-lg">
            <nav className="container py-4 flex flex-col gap-1">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href}>
                  <span className="block py-3 px-2 text-[#3B2F2A] font-body font-medium text-base border-b border-[#F7F3EE] cursor-pointer">
                    {link.label}
                  </span>
                </Link>
              ))}
